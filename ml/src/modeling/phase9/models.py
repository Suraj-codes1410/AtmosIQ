"""
AtmosIQ Phase 9: Temporal Deep-Learning Architectures (LSTM, TCN, Temporal Transformer).
"""
...
class BasePhase9Model:
    """Base class for Phase 9 temporal deep-learning architectures."""
...
class Phase9LSTMModel(BasePhase9Model):
    ...
    def compute_loss_and_backward(self, X: np.ndarray, y: np.ndarray) -> Tuple[float, Dict[str, np.ndarray]]:
        ...
        dW_out = h_final.T @ dL_dz
        db_out = np.sum(dL_dz, axis=0)

        dh = dL_dz @ self.params["W_out"].T
...
class Phase9TCNModel(BasePhase9Model):
...
class Phase9TransformerModel(BasePhase9Model):
    ...
    def compute_loss_and_backward(self, X: np.ndarray, y: np.ndarray) -> Tuple[float, Dict[str, np.ndarray]]:
        ...
        dpool = dz_ff @ self.params["W_ff"].T + dz_ff
        dattn_out = np.repeat(dpool[:, None, :] / W, W, axis=1)
