...
logger = setup_logger("ProvenancePhase6F")


class ProvenanceVerifierPhase6F:
    """
    Cryptographic Provenance Verifier for Phase 6F.
    Verifies immutable hashes of Datasets v1, v2, v3, Control Model v1, Production Model v3,
    Feature Registry, and Phase 6D production uncertainty layer.
    """
    ...
    def __init__(self, root_dir: Path = ROOT_DIR):
        ...
        self.feat_reg_path = self.root_dir / "ml" / "models" / "production" / "v3" / "feature_registry.csv"
        self.prod_unc_path = self.root_dir / "ml" / "uncertainty" / "production" / "v1" / "uncertainty_method.json"
        self.prod_calib_path = self.root_dir / "ml" / "uncertainty" / "production" / "v1" / "calibration_artifacts.json"
    ...
    def verify_all(self) -> dict:
        logger.info("Verifying Upstream Immutable Artifact Hashes for Phase 6F...")
        ...
        feat_reg_h = self.calculate_sha256(self.feat_reg_path)

        assert self.prod_unc_path.exists(), f"Production uncertainty layer missing at {self.prod_unc_path}!"
        assert self.prod_calib_path.exists(), f"Production calibration artifacts missing at {self.prod_calib_path}!"
        ...
        assert all_passed, "Upstream provenance verification failed in Phase 6F!"
        ...
        return {
            ...
            "feature_registry_hash": feat_reg_h,
            "production_uncertainty_layer": "ml/uncertainty/production/v1",
