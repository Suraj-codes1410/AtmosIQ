"""
AtmosIQ Phase 8B: Physics Constraint Validation Engine.
"""
...
class Phase8BPhysicsValidator:
    """Validates physical boundary constraints for synthetic trajectories."""
    ...
    def validate_trajectory(self, df_traj: pd.DataFrame, trajectory_id: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        if bad_vi > 0: violations.append(f"Hydrodynamic identity mismatch in VI ({bad_vi} rows)")

        # 9. Rain Event Binary Logic
        bad_rain_event = int(((df_traj["rain_event_1d"] == 1) != (df_traj["rainfall_1d"] >= 1.0)).sum())
        if bad_rain_event > 0: violations.append(f"Rain event indicator logic mismatch ({bad_rain_event} rows)")

        # 10. NaN / Inf Prevention
        nan_count = int(df_traj.isna().sum().sum())
        if nan_count > 0: violations.append(f"Contains {nan_count} NaN values")
        inf_count = int(np.isinf(df_traj.select_dtypes(include=[np.number]).values).sum())
        if inf_count > 0: violations.append(f"Contains {inf_count} infinite values")
